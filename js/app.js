(() => {
  ...
  window.addEventListener("load", function() {
    ...
    const translateScreen = () => {
      if (opacity == 0)
        main_navigation.style.transform = `translateX(${window.innerWidth}px)`;
    };
    ...
    let toggleNav = function(e) {
      ...
      main_navigation.style.transform = opacity == 0 ? `translateX(${window.innerWidth}px)` : `translateX(0px)`;
